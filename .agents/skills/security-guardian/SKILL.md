...
3. **Integrates with Codex hooks** (rtk-rewrite.sh, rtk-suggest.sh)
...
# Verify running in Codex context
if [ -z "$CLAUDE_CODE_HOOK_BASH_TEMPLATE" ]; then
    echo "Error: Not running in Codex context"
    exit 1
fi
...
- [ ] **Source validation**: Only execute hooks from `.Codex/hooks/`
...
# Hook security
rg "eval|source" --type bash .Codex/hooks/
```
